...
import { MOCK_CURRENT_DATE, MOCK_CURRENT_TIMESTAMP_SEC } from './setup';
...
describe('getTokenExpirationTimeMs', () => {
  ...
  describe('when the token is valid', () => {
    it('should return the correct time remaining in milliseconds for a token that expires in the future', () => {
      ...
      const futureExpSeconds = MOCK_CURRENT_TIMESTAMP_SEC + oneHourInSeconds; // Expires in 1 hour
  ...
  describe('when the token is invalid', () => {
    it('should return a negative value for a token that has already expired', () => {
      ...
      const oneHourInSeconds = 3600;
      const pastExpSeconds = MOCK_CURRENT_TIMESTAMP_SEC - oneHourInSeconds; // Expired 1 hour ago
